...
class XSSerOptions(optparse.OptionParser):
    def __init__(self, *args):
        optparse.OptionParser.__init__(self, 
                           ...
                           prog='XSSer.py',
                           version='\033[1;35mXSSer v0.6a\033[1;m - (Copyright - GPL3.0) - 2010 \033[1;35mby psy\033[1;m\n',
                           usage= '\npython XSSer.py [-u <url> |-i <file> |-g <dork>] [-p <postdata> |-c <crawling>] [OPTIONS] [Request] [Bypassing] [Techniques]\n')
        ...
        #self.add_option("--update", action="store", dest="update", help="update XSSer tool ( framework + vectors )") 
        ...
        group1.add_option("-g", action="store", dest="dork", help="Process search engine dork results as target urls (ex: inurl:vulnerable.asp?id=) ")
        group1.add_option("--Ge", action="store", dest="dork_engine", help="Search engine to use for dorking (scroogle, duck, altavista, bing)")
        self.add_option_group(group1)

        group2 = optparse.OptionGroup(self, "*HTTP POST data*",
